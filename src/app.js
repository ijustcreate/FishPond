...
document.getElementById('add-fish-button').addEventListener('click', () => { const index = state.fish.length; const fish = makeFish({ id: uid('fish'), name: `Nova ${index + 1}`, x: .5, y: .55, size: .55, color: '#83b9c2', secondary: '#eee5d8', hunger: 35 }, index); state.fish.push(fish); ensureSocialRelationships(); renderFishRoster(); renderPondLibrary(); saveActivePond(); showToast(`${fish.name} is staying near cover while settling in`); });
...
window.render_game_to_text = () => JSON.stringify({
  ...
  ui: { headerRemoved: !document.querySelector('.stage-head'), collectionRemoved: !document.querySelector('[data-view="collection"]'), timeDialLocation: document.getElementById('time-dial').closest('.left-sidebar') ? 'left-sidebar' : 'stage', fishRosterOpen: !document.getElementById('fish-roster-panel').hidden, fishRosterCount: document.querySelectorAll('.fish-roster-card').length, fishRosterGroups: Object.fromEntries([...document.querySelectorAll('.fish-roster-group')].map((group) => [group.dataset.fishGroup, group.querySelectorAll('.fish-roster-card').length])), pondActions: 'right-click or long-press context menu', pondContextOpen: !document.getElementById('pond-context-menu').hidden, pondContextTarget: pondContextTargetId },

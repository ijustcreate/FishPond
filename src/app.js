...
window.render_game_to_text = () => JSON.stringify({
  ...
  ui: { headerRemoved: !document.querySelector('.stage-head'), collectionRemoved: !document.querySelector('[data-view="collection"]'), timeDialLocation: document.getElementById('time-dial').closest('.left-sidebar') ? 'left-sidebar' : 'stage', fishRosterOpen: !document.getElementById('fish-roster-panel').hidden, fishRosterCount: document.querySelectorAll('.fish-roster-card').length, fishRosterGroups: Object.fromEntries([...document.querySelectorAll('.fish-roster-group')].map((group) => [group.dataset.fishGroup, group.querySelectorAll('.fish-roster-card').length])), fishCreator: { open: !fishCreator.hidden, step: fishCreatorStep + 1, species: fishCreatorDraft.species, color: fishCreatorDraft.color, name: fishCreatorDraft.name.trim() }, pondActions: 'right-click or long-press context menu', pondContextOpen: !document.getElementById('pond-context-menu').hidden, pondContextTarget: pondContextTargetId },

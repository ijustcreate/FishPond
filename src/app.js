...
const defaultFoodTiming = (id) => id === 'food-algae' ? { lifetime: 240, floatTime: 3, sinkDuration: 42 } : id === 'food-krill' ? { lifetime: 135, floatTime: 8, sinkDuration: 58 } : { lifetime: 165, floatTime: 24, sinkDuration: 72 };
function normalizeFoodPreset(food) { const defaults = defaultFoodTiming(food.id); food.lifetime = clamp(Number(food.lifetime ?? defaults.lifetime), 20, 600); food.floatTime = clamp(Number(food.floatTime ?? defaults.floatTime), 0, 90); food.sinkDuration = clamp(Number(food.sinkDuration ?? defaults.sinkDuration), 18, 180); food.tags = normalizeFoodTags(Array.isArray(food.tags) ? food.tags : defaultPresetTags(food.id)); return food; }
...
function makeAnatomy(config = {}) {
  ...
  return {
    fins: {
      ...
      dorsal: { ...profile(.9, .82, [[.18, .32], [.48, .78], [.78, .48]]), position: .3 },
...
const state = {
  ...
  foodPresets: [
    { id: 'food-gold', name: 'Golden Growth', color: '#d8a84f', size: 3, count: 10, tags: ['pellet','growth','omnivore'], lifetime: 165, floatTime: 24, sinkDuration: 72 },
    { id: 'food-krill', name: 'Crimson Krill', color: '#c85d4d', size: 2.4, count: 8, tags: ['krill','protein','treat'], lifetime: 135, floatTime: 8, sinkDuration: 58 },
    { id: 'food-algae', name: 'Algae Wafer', color: '#739352', size: 5.2, count: 4, tags: ['algae','wafer','bottom-feeder','pleco-only'], lifetime: 240, floatTime: 3, sinkDuration: 42 }
  ],
...
function applyPondData(data) {
  ...
  state.fish.forEach((fish, index) => { fish.foodTags = normalizeFoodTags(Array.isArray(fish.foodTags) ? fish.foodTags : defaultFishFoodTags(fish.species, fish.algaeEater)); fish.physiology.digestiveLoad ??= .08; fish.physiology.nextWasteAt ??= 32 + seeded(index + 301) * 38; fish.cognition.hideCaveId ??= null; fish.cognition.caveStage ??= null; fish.cognition.caveRestUntil ??= 0; fish.anatomy.barbelLength ??= 1; fish.ambientTrailAccumulator = 0; }); ensureSocialRelationships(); state.foodPresets.forEach(normalizeFoodPreset); state.foods.forEach((piece) => { const preset = state.foodPresets.find((food) => food.id === piece.foodId); const timing = preset || defaultFoodTiming(piece.foodId); piece.floatTime ??= timing.floatTime; piece.lifetime ??= timing.lifetime; piece.sinkRate ??= 1 / (timing.sinkDuration || 60); }); state.ambientTrails = [];
...
let pondContextTargetId = null;
let suppressPondClickId = null;
function closePondContextMenu({ restoreFocus = false } = {}) { const menu = document.getElementById('pond-context-menu'); if (!menu || menu.hidden) return; const targetId = pondContextTargetId; menu.hidden = true; pondContextTargetId = null; if (restoreFocus && targetId) document.querySelector(`[data-pond-id="${CSS.escape(targetId)}"]`)?.focus(); }
function openPondContextMenu(pondId, clientX, clientY, { focus = false } = {}) {
  const pond = state.ponds.find((item) => item.id === pondId), menu = document.getElementById('pond-context-menu'); if (!pond || !menu) return;
  closePondContextMenu(); pondContextTargetId = pondId; menu.hidden = false; menu.setAttribute('aria-label', `Actions for ${pond.name}`);
  const remove = document.getElementById('pond-context-delete'); remove.disabled = state.ponds.length <= 1; remove.setAttribute('aria-disabled', String(remove.disabled));
  const rect = menu.getBoundingClientRect(), margin = 8; menu.style.left = `${clamp(clientX, margin, window.innerWidth - rect.width - margin)}px`; menu.style.top = `${clamp(clientY, margin, window.innerHeight - rect.height - margin)}px`;
  if (focus) document.getElementById('pond-context-rename').focus();
}
function bindPondContextTrigger(button, pondId) {
  let holdTimer = 0, holdX = 0, holdY = 0;
  const cancelHold = () => { clearTimeout(holdTimer); holdTimer = 0; };
  button.addEventListener('contextmenu', (event) => { event.preventDefault(); openPondContextMenu(pondId, event.clientX, event.clientY); });
  button.addEventListener('pointerdown', (event) => { if (event.pointerType !== 'touch') return; holdX = event.clientX; holdY = event.clientY; cancelHold(); holdTimer = setTimeout(() => { suppressPondClickId = pondId; navigator.vibrate?.(18); openPondContextMenu(pondId, holdX, holdY); }, 520); });
  button.addEventListener('pointermove', (event) => { if (Math.hypot(event.clientX - holdX, event.clientY - holdY) > 10) cancelHold(); });
  ...
  button.addEventListener('keydown', (event) => { if (event.key === 'ContextMenu' || (event.shiftKey && event.key === 'F10')) { event.preventDefault(); const rect = button.getBoundingClientRect(); openPondContextMenu(pondId, rect.left + 18, rect.bottom - 4, { focus: true }); } });
}
function renderPondLibrary() {
  const host = document.getElementById('pond-library'); if (!host) return; closePondContextMenu(); host.replaceChildren();
  state.ponds.forEach((pond) => { const row = document.createElement('div'); row.className = 'pond-library-row'; const button = document.createElement('button'); button.className = `pond-library-item${pond.id === state.activePondId ? ' active' : ''}`; button.dataset.pondId = pond.id; button.title = `Open ${pond.name}. Right-click or long-press for Rename and Delete`; button.setAttribute('aria-description', 'Right-click or long-press for Rename and Delete'); button.innerHTML = `<i></i><span>${pond.name}</span><small>${pond.data.fish.length}</small>`; button.addEventListener('click', () => { if (suppressPondClickId === pond.id) { suppressPondClickId = null; return; } switchPond(pond.id); }); bindPondContextTrigger(button, pond.id); row.append(button); host.appendChild(row); });
  const current = state.ponds.find((pond) => pond.id === state.activePondId); if (current) { const rosterTitle = document.getElementById('fish-roster-title'); if (rosterTitle) rosterTitle.textContent = current.name; } const badge = document.getElementById('fish-count-badge'); if (badge) badge.textContent = state.fish.length; if (typeof installInfoTooltips === 'function') queueMicrotask(installInfoTooltips);
}
...
function starterEdgePoint(index, count, inset, seed) { const perimeter = (index / count * 4 + seeded(seed + index * 4.1) * .42) % 4, side = Math.floor(perimeter), t = .1 + (perimeter - side) * .8; if (side === 0) return { x: t, y: inset }; if (side === 1) return { x: 1 - inset, y: t }; if (side === 2) return { x: 1 - t, y: 1 - inset }; return { x: inset, y: 1 - t }; }
function createStarterPondData(name) {
  const seed = [...name].reduce((sum, character) => sum + character.charCodeAt(0), state.ponds.length * 37 + 11), base = capturePondData(), rockCount = 10, lilyCount = 2 + Math.floor(seeded(seed + 2) * 3), reedCount = 5 + Math.floor(seeded(seed + 7) * 5), decorations = [];
  for (let i = 0; i < rockCount; i++) { const point = starterEdgePoint(i, rockCount, -.008, seed + 20); decorations.push({ id: uid('rock'), type: 'rock', ...point, scale: .75 + seeded(seed + i * 3) * .72, rotation: seeded(seed + i * 8) * TAU }); }
  for (let i = 0; i < lilyCount; i++) { const point = starterEdgePoint(i + .55, lilyCount, .115 + seeded(seed + i) * .025, seed + 40); decorations.push({ id: uid('lily'), type: 'lily', ...point, scale: .72 + seeded(seed + i * 9) * .42, rotation: seeded(seed + i * 11) * TAU, age: 16 + seeded(seed + i * 5) * 65, lifespan: 125 + seeded(seed + i * 7) * 60, health: .88, growth: 1, sway: seeded(seed + i * 13) * TAU, bloom: seeded(seed + i * 17) > .55, dead: false, prunedAt: -99 }); }
  for (let i = 0; i < reedCount; i++) { const point = starterEdgePoint(i + .28, reedCount, .045 + seeded(seed + i * 2) * .018, seed + 80); decorations.push({ id: uid('reed'), type: 'reed', ...point, scale: .72 + seeded(seed + i * 6) * .55, rotation: seeded(seed + i * 10) * TAU }); }
  const koi = makeFish({ id: uid('koi'), name: 'New Koi', species: 'Koi', variety: 'Kohaku', x: .42, y: .45, z: .34, size: .82, color: '#de6d36', secondary: '#f1eee4', pattern: 'spotted', hunger: 48, sociability: .78, curiosity: .66, foodDrive: .72 }, seed + 1);
  const pleco = makeFish({ id: uid('pleco'), name: 'New Pleco', species: 'Plecostomus', variety: 'Bristlenose', x: .76, y: .72, z: .88, size: .66, color: '#5d5544', secondary: '#9b8966', finColor: '#4b4134', pattern: 'stripe', hunger: 34, activity: .23, boldness: .18, sociability: .08, preferredDepth: .91, algaeEater: true, barbelsEnabled: false, scalesEnabled: true, scaleColor: '#b7a475' }, seed + 2);
  ensureSocialRelationships([koi, pleco]);
  return { ...base, selectedId: null, fish: [koi, pleco], decorations, terrain: [{ x: .5, y: .5, radius: .38, depth: .34 }, { x: .22, y: .3, radius: .17, depth: -.08 }], foods: [], wastes: [], wasteDarkening: 0, algaeLevel: .1, dragonflies: base.dragonflies.map((fly) => ({ ...fly, landing: null, state: 'perched', vx: 0, vy: 0 })) };
}
function openPondManager(mode = 'create', pondId = state.activePondId) { pondManagerMode = mode; pondManagerTargetId = mode === 'rename' ? pondId : null; const target = state.ponds.find((pond) => pond.id === pondManagerTargetId); document.getElementById('pond-manager-title').textContent = mode === 'create' ? 'Create a pond' : `Rename ${target?.name || 'pond'}`; document.getElementById('pond-name-input').value = mode === 'create' ? generatedPondName() : target?.name || ''; document.getElementById('pond-delete-current').hidden = mode === 'create' || state.ponds.length <= 1; document.getElementById('pond-manager').hidden = false; document.getElementById('pond-name-input').focus(); document.getElementById('pond-name-input').select(); }
function closePondManager() { document.getElementById('pond-manager').hidden = true; }
function commitPondManager() { const name = document.getElementById('pond-name-input').value.trim() || generatedPondName(); if (pondManagerMode === 'rename') { const pond = state.ponds.find((item) => item.id === pondManagerTargetId); if (pond) pond.name = name; } else { saveActivePond(); const data = createStarterPondData(name), id = uid('pond'); state.ponds.push({ id, name, data }); state.activePondId = id; applyPondData(data); } renderPondLibrary(); persistPonds(); closePondManager(); showToast(`${name} saved`); }
...
function ensureSocialRelationships(fishList = state.fish) {
  fishList.forEach((fish) => { fish.social ||= { relationships: {} }; fish.social.relationships ||= {}; fishList.forEach((other) => { if (fish === other) return; fish.social.relationships[other.id] ||= { familiarity: .16, affinity: seeded(fish.phase + other.phase), trust: .24, foodCompetition: .12, avoidance: fish.algaeEater || other.algaeEater ? .3 : .08, followBias: fish.species === 'Koi' && other.species === 'Koi' ? .48 : .12 }; }); });
}
...
function mouthPointNormalized(fish) {
  const width = Math.max(1, canvas.clientWidth), height = Math.max(1, canvas.clientHeight), depthScale = clamp(1.04 - fish.z * (.42 * state.pondDepth), .38, 1.04), climb = clamp(-fish.verticalVelocity * 3.5, -.16, .16);
  const noseLength = (fish.algaeEater ? 6 : 10.5) * fish.size * depthScale * fish.anatomy.headPoint * (1 + climb), mouthInset = 1.6 * fish.size * depthScale, reach = Math.max(1.5, noseLength - mouthInset);
  return { x: fish.x + Math.cos(fish.heading) * reach / width, y: fish.y + Math.sin(fish.heading) * reach / height };
}
function feedingHeadTarget(fish, food) { const mouth = mouthPointNormalized(fish); return { x: clamp(food.x - (mouth.x - fish.x), .045, .955), y: clamp(food.y - (mouth.y - fish.y), .07, .93) }; }
...
function targetForIntent(fish, intent) {
  ...
  if (intent === 'hide') {
    const caves = state.decorations.filter((item) => item.type === 'cave'); const remembered = caves.find((cave) => cave.id === fish.cognition.hideCaveId); const cover = remembered || nearestItem(fish, caves.length ? caves : state.decorations.filter((item) => item.type === 'lily'));
    fish.cognition.hideCaveId = cover?.type === 'cave' ? cover.id : null;
    if (cover?.type === 'cave') { if (caveChamberContains(fish, cover) || fish.cognition.caveStage === 'enter') { fish.cognition.caveStage = 'enter'; return { x: cover.x, y: cover.y }; } const entrance = caveEntrancePoint(cover); if (Math.hypot(fish.x - entrance.x, fish.y - entrance.y) < .025 * cover.scale) fish.cognition.caveStage = 'enter'; else fish.cognition.caveStage = 'approach'; return fish.cognition.caveStage === 'enter' ? { x: cover.x, y: cover.y } : entrance; }
    fish.cognition.caveStage = null; return cover ? { x: cover.x, y: cover.y } : { x: .14, y: .28 };
  }
  if (intent === 'shoal') {
    const companions = state.fish.filter((other) => other !== fish && !other.algaeEater);
    const companion = [...companions].sort((a, b) => { const ar = fish.social.relationships[a.id], br = fish.social.relationships[b.id]; return ((br?.affinity || 0) + (br?.trust || 0)) - ((ar?.affinity || 0) + (ar?.trust || 0)); })[0] || nearestItem(fish, companions);
...
function evaluateIntent(fish) {
  ...
  const scores = [
    ...
    { type: 'hide', score: (body.stress / 100) * p.startleSensitivity + fish.preferences.shadePreference * (state.day ? .18 : .04) + (state.decorations.some((item) => item.type === 'cave') ? .1 + (1 - body.energy / 100) * .25 : 0), reason: 'a sheltered cave feels safe' },
...
function updatePhysiology(dt) {
  ...
  state.fish.forEach((fish) => {
    ...
    if (fish.species === 'Koi') state.fish.filter((other) => other !== fish && other.species === 'Koi').forEach((other) => { const relation = fish.social.relationships[other.id]; if (!relation) return; const close = Math.hypot(other.x - fish.x, other.y - fish.y) < .22; relation.familiarity = clamp(relation.familiarity + elapsed * (close ? .0015 : .00008), 0, 1); relation.trust = clamp(relation.trust + elapsed * (close && fish.physiology.stress < 45 ? .0009 : .00005), 0, 1); relation.affinity = clamp(relation.affinity + elapsed * (close ? .00055 : 0), 0, 1); relation.followBias = clamp(.2 + relation.familiarity * .55 + relation.trust * .25, 0, 1); });
  });
}
...
function updateFood(dt) {
  state.foods.forEach((piece) => {
    ...
    piece.age += dt;
    if (piece.age < piece.floatTime) { const surfaceTarget = .012 + Math.sin(piece.age * 1.7 + piece.spin) * .003; piece.depth = lerp(piece.depth, surfaceTarget, 1 - Math.exp(-3.2 * dt)); }
    else piece.depth = clamp(piece.depth + piece.sinkRate * dt, 0, 1);
    piece.spin += dt * piece.spinRate;
  });
  state.foods = state.foods.filter((piece) => piece.age < piece.lifetime && !piece.eaten);
}
...
function caveEntrancePoint(cave) { const distance = .068 * cave.scale, angle = cave.rotation || 0; return { x: clamp(cave.x + Math.cos(angle) * distance, .045, .955), y: clamp(cave.y + Math.sin(angle) * distance, .07, .93) }; }
...
function updateFish(fish, dt) {
  const body = fish.physiology;
  const restingCave = state.decorations.find((item) => item.id === fish.cognition.hideCaveId && item.type === 'cave');
  if (fish.cognition.caveRestUntil && state.clock >= fish.cognition.caveRestUntil) { fish.cognition.caveRestUntil = 0; fish.cognition.caveStage = null; fish.cognition.hideCaveId = null; fish.cognition.commitmentUntil = 0; fish.cognition.intent = 'cruise'; fish.cognition.target = targetForIntent(fish, 'cruise'); }
  if (restingCave && fish.cognition.caveRestUntil > state.clock) { fish.cognition.intent = 'hide'; fish.cognition.target = { x: restingCave.x, y: restingCave.y }; fish.x = lerp(fish.x, restingCave.x, clamp(dt * 1.4, 0, 1)); fish.y = lerp(fish.y, restingCave.y, clamp(dt * 1.4, 0, 1)); fish.z = lerp(fish.z, .68, clamp(dt * 1.2, 0, 1)); fish.speed = 0; fish.desiredSpeed = 0; fish.tailBeat *= Math.max(0, 1 - dt * 8); body.stress = clamp(body.stress - dt * .8, 0, 100); return; }
  ...
  if (fish.cognition.intent === 'feed') {
    const food = foodTargetFor(fish);
    if (food) fish.cognition.target = feedingHeadTarget(fish, food);
  ...
  state.fish.forEach((other) => {
    ...
    const solitarySpace = fish.algaeEater || other.algaeEater ? 1.38 : 1; const personalSpace = (ownShape.radius + otherShape.radius) * (1.32 + body.stress / 180) * solitarySpace;
  ...
  if (fish.cognition.intent === 'hide' && fish.cognition.hideCaveId) { const cave = state.decorations.find((item) => item.id === fish.cognition.hideCaveId); if (cave && caveChamberContains(fish, cave)) { fish.cognition.caveStage = 'inside'; fish.cognition.caveRestUntil = state.clock + 9 + seeded(fish.phase + state.clock) * 9; fish.cognition.commitmentUntil = fish.cognition.caveRestUntil; fish.speed = 0; } }
  const food = activeFood;
  const mouth = mouthPointNormalized(fish); const mouthDistance = food ? Math.hypot((food.x - mouth.x) * Math.max(1, canvas.clientWidth), (food.y - mouth.y) * Math.max(1, canvas.clientHeight)) : Infinity;
  if (food && fish.cognition.intent === 'feed' && mouthDistance < 3.2 + fish.size * 2.4 && Math.abs(food.depth - fish.z) < .2 && body.satiety < .94) {
    food.eaten = true;
    ...
    fish.cognition.memories.unshift({ type: 'fed', x: food.x, y: food.y, time: state.clock, emotionalWeight: .7, mouthErrorPx: mouthDistance });
...
function updateSelectionTrail(dt) {
  if (!state.selectedId) { state.selectionTrail = []; state.trailAccumulator = 0; return; }
...
function finFrame(fish, rig, widths, type, side = 1, visualScale = 1) {
  ...
  const bodyAnchor = type === 'dorsal' ? pointOnBody(rig, widths, profile.position ?? .3, 0) : null;
  const angle = bodyAnchor?.angle ?? rig.angles[setup.joint];
  const anchor = bodyAnchor || rig.joints[setup.joint];
...
function drawFoodPiece(piece, width, height) {
  ...
  const depthScale = lerp(1, .34, Math.pow(piece.depth, .78));
...
function renderPond() {
  ...
  [...state.fish].sort((a, b) => b.z - a.z).forEach((fish) => {
    ...
    const underwaterAlpha = lerp(.58, .94, state.waterSettings.clarity) * (1 - fish.z * .16), depthBrightness = lerp(1.02, .48, Math.pow(fish.z, .82));
    ctx.save(); ctx.filter = `brightness(${depthBrightness}) saturate(${lerp(1.04, .72, fish.z)})`; drawProceduralFish(ctx, fish, rig, depthScale, { alpha: underwaterAlpha }); ctx.restore();
  });
...
function renderFishRoster() {
  ...
  const groupedFish = [
    { id: 'koi', label: 'Koi', fish: state.fish.filter((fish) => !fish.algaeEater && fish.species !== 'Plecostomus') },
    { id: 'plecos', label: 'Plecos', fish: state.fish.filter((fish) => fish.algaeEater || fish.species === 'Plecostomus') }
  ];
  groupedFish.forEach((group) => {
    const section = document.createElement('section'); section.className = 'fish-roster-group'; section.dataset.fishGroup = group.id; section.setAttribute('aria-labelledby', `fish-group-${group.id}`);
    const heading = document.createElement('div'); heading.className = 'fish-roster-group-head'; heading.innerHTML = `<h3 id="fish-group-${group.id}">${group.label}</h3><span>${group.fish.length}</span>`;
    const cards = document.createElement('div'); cards.className = 'fish-roster-cards'; section.append(heading, cards); host.appendChild(section);
    ...
    const preview = document.createElement('canvas'); preview.width = 230; preview.height = 92; const info = document.createElement('div'); info.className = 'fish-roster-info'; info.innerHTML = `<div><strong>${fish.name}</strong><span>${fish.species} · ${fish.variety}</span></div><em>${fish.cognition.intent === 'feed' ? 'feeding' : fish.attached ? 'resting' : 'swimming'}</em>`; const rename = document.createElement('button'); rename.className = 'fish-card-rename'; rename.dataset.renameFishId = fish.id; rename.textContent = 'Rename'; rename.title = `Rename ${fish.name}`; const remove = document.createElement('button'); remove.className = 'fish-card-remove'; remove.dataset.removeFishId = fish.id; remove.title = `Remove ${fish.name}`; remove.textContent = '×'; card.append(preview, info, rename, remove); cards.appendChild(card);
...
const fishCreator = document.getElementById('fish-creator');
const fishCreatorPreview = document.getElementById('fish-create-preview');
const fishCreatorPreviewCtx = fishCreatorPreview.getContext('2d');
const fishCreatorColorNames = { '#df7037': 'Ember orange', '#e2c64f': 'Golden yellow', '#f0eee4': 'Pearl white', '#7da7b5': 'Mist blue', '#614f3d': 'River brown' };
let fishCreatorStep = 0;
let fishCreatorDraft = { species: 'koi', color: '#df7037', name: '' };
function fishCreatorSpeciesConfig() {
  if (fishCreatorDraft.species === 'pleco') return { species: 'Plecostomus', variety: 'Bristlenose', algaeEater: true, size: .68, secondary: '#d8cfb0', z: .82, preferredDepth: .84, sociability: .18, territoriality: .72, pattern: 'spotted', scalesEnabled: true, barbelsEnabled: false };
  return { species: 'Koi', variety: 'Kohaku', algaeEater: false, size: .72, secondary: '#f0eadc', z: .38, preferredDepth: .4, sociability: .74, territoriality: .2, pattern: 'spotted', barbelsEnabled: true };
}
function creatorPreviewFish() { const species = fishCreatorSpeciesConfig(); return makeFish({ id: 'creator-preview', name: fishCreatorDraft.name.trim() || (fishCreatorDraft.species === 'pleco' ? 'New Pleco' : 'New Koi'), x: .5, y: .5, color: fishCreatorDraft.color, finColor: fishCreatorDraft.color, hunger: 35, ...species }, state.fish.length + 71); }
function renderFishCreatorPreview() {
  const fish = creatorPreviewFish(); drawPanelBackdrop(fishCreatorPreviewCtx, fishCreatorPreview.width, fishCreatorPreview.height); const fit = fish.algaeEater ? 1.02 : 1.08; const rig = straightRig(350, 90, 23.5 * fish.size, 0); drawProceduralFish(fishCreatorPreviewCtx, fish, rig, fit, { preview: true });
  document.getElementById('fish-create-summary-name').textContent = fish.name; document.getElementById('fish-create-summary-species').textContent = fish.species === 'Plecostomus' ? 'Pleco' : 'Koi'; const colorLabel = document.getElementById('fish-create-summary-color'); colorLabel.lastChild.textContent = ` ${fishCreatorColorNames[fishCreatorDraft.color] || fishCreatorDraft.color.toUpperCase()}`; colorLabel.style.setProperty('--fish-create-color', fishCreatorDraft.color);
}
function syncFishCreator() {
  document.querySelectorAll('[data-creator-step]').forEach((stage) => stage.classList.toggle('active', Number(stage.dataset.creatorStep) === fishCreatorStep));
  document.querySelectorAll('[data-creator-progress]').forEach((marker) => { const progress = Number(marker.dataset.creatorProgress); marker.classList.toggle('active', progress <= fishCreatorStep); marker.classList.toggle('current', progress === fishCreatorStep); });
  document.querySelectorAll('[data-create-species]').forEach((button) => button.classList.toggle('active', button.dataset.createSpecies === fishCreatorDraft.species));
  document.querySelectorAll('[data-create-color]').forEach((button) => button.classList.toggle('active', button.dataset.createColor === fishCreatorDraft.color));
  document.getElementById('fish-create-color').value = fishCreatorDraft.color; document.getElementById('fish-create-back').hidden = fishCreatorStep === 0; document.getElementById('fish-create-next').hidden = fishCreatorStep === 3; document.getElementById('fish-create-confirm').hidden = fishCreatorStep !== 3;
  const titles = ['Choose a fish', 'Choose a base color', 'Name your fish', 'Ready for the pond']; document.getElementById('fish-creator-title').textContent = titles[fishCreatorStep]; document.getElementById('fish-create-error').textContent = '';
  if (fishCreatorStep === 2) queueMicrotask(() => document.getElementById('fish-create-name').focus()); if (fishCreatorStep === 3) renderFishCreatorPreview();
}
function openFishCreator() { fishCreatorStep = 0; fishCreatorDraft = { species: 'koi', color: '#df7037', name: '' }; document.getElementById('fish-create-name').value = ''; fishCreator.hidden = false; syncFishCreator(); }
function closeFishCreator() { fishCreator.hidden = true; }
function advanceFishCreator() { if (fishCreatorStep === 2) { fishCreatorDraft.name = document.getElementById('fish-create-name').value.trim(); if (!fishCreatorDraft.name) { document.getElementById('fish-create-error').textContent = 'Please give your fish a name before continuing.'; document.getElementById('fish-create-name').focus(); return; } } fishCreatorStep = Math.min(3, fishCreatorStep + 1); syncFishCreator(); }
function createFishFromWizard() {
  const index = state.fish.length, species = fishCreatorSpeciesConfig(), edgeSide = seeded(index + 91) > .5 ? .82 : .18;
  const fish = makeFish({ id: uid('fish'), name: fishCreatorDraft.name.trim(), x: fishCreatorDraft.species === 'pleco' ? edgeSide : .38 + seeded(index + 82) * .24, y: .36 + seeded(index + 83) * .28, color: fishCreatorDraft.color, finColor: fishCreatorDraft.color, hunger: 35, ...species }, index);
  state.fish.push(fish); ensureSocialRelationships(); renderFishRoster(); renderPondLibrary(); saveActivePond(); closeFishCreator(); requestAnimationFrame(() => document.querySelector(`[data-fish-id="${CSS.escape(fish.id)}"]`)?.scrollIntoView({ block: 'center', behavior: 'smooth' })); showToast(`${fish.name} joined ${fish.algaeEater ? 'the plecos' : 'the koi'}`);
}
...
function closeFoodEditor() { document.getElementById('food-editor').hidden = true; }
function openFoodEditor(foodId) { state.selectedFoodId = foodId; renderFoodPresets(); syncFoodEditor(); document.getElementById('food-editor').hidden = false; document.getElementById('food-name').focus(); }
function renderFoodPresets() {
  const host = document.getElementById('food-presets'); if (!host) return; host.replaceChildren();
  state.foodPresets.forEach((food) => { const wrap = document.createElement('div'); wrap.className = 'food-preset-wrap'; const button = document.createElement('button'); button.className = `food-preset${food.id === state.selectedFoodId ? ' active' : ''}`; button.style.setProperty('--food-color', food.color); const dot = document.createElement('i'), name = document.createTextNode(food.name), tags = document.createElement('small'); tags.textContent = normalizeFoodTags(food.tags).join(' · '); button.append(dot, name, tags); button.addEventListener('click', () => { state.selectedFoodId = food.id; closeFoodEditor(); renderFoodPresets(); setFoodMode(true); }); const edit = document.createElement('button'); edit.className = 'food-preset-edit'; edit.textContent = 'Edit'; edit.setAttribute('aria-label', `Edit ${food.name}`); edit.addEventListener('click', () => openFoodEditor(food.id)); wrap.append(button, edit); host.appendChild(wrap); });
}
function syncFoodEditor() { const food = selectedFoodPreset(); if (!food) return; normalizeFoodPreset(food); document.getElementById('food-name').value = food.name; document.getElementById('food-color').value = food.color; document.getElementById('food-size').value = food.size; document.getElementById('food-count').value = food.count; document.getElementById('food-lifetime').value = food.lifetime; document.getElementById('food-float-time').value = food.floatTime; document.getElementById('food-tags').value = normalizeFoodTags(food.tags).join(', '); }
function openFoodPanel() { closeFishRoster(); closeWaterSettings(); closeDecorator(); closeFoodEditor(); document.getElementById('food-panel').hidden = false; renderFoodPresets(); setFoodMode(true); }
function closeFoodPanel() { const panel = document.getElementById('food-panel'); const wasActive = !panel.hidden || state.interaction === 'drop-food'; closeFoodEditor(); panel.hidden = true; if (wasActive) setFoodMode(false); }
...
function dropFood(x, y) {
  const preset = normalizeFoodPreset(selectedFoodPreset()); const count = preset.count;
  for (let i = 0; i < count; i++) {
    const angle = seeded(i + state.clock) * TAU;
    const radius = .006 + seeded(i * 2.8 + x) * .023;
    state.foods.push({ id: uid('pellet'), foodId: preset.id, color: preset.color, x: clamp(x + Math.cos(angle) * radius, .03, .97), y: clamp(y + Math.sin(angle) * radius, .04, .96), depth: .008 + seeded(i + y) * .008, floatTime: preset.floatTime * (.9 + seeded(i + 17) * .2), lifetime: preset.lifetime * (.94 + seeded(i + 23) * .12), sinkRate: (1 / preset.sinkDuration) * (.86 + seeded(i + 3) * .28), size: preset.size * (.84 + seeded(i + 8) * .3), age: 0, spin: seeded(i + 5) * TAU, spinRate: (seeded(i + 12) - .5) * 2, vx: 0, vy: 0, eaten: false, claimedBy: null });
  }
...
document.getElementById('food-edit-cancel').addEventListener('click', closeFoodEditor);
document.getElementById('food-save').addEventListener('click', () => { const food = selectedFoodPreset(); if (!food) return; food.name = document.getElementById('food-name').value.trim() || food.name; food.color = document.getElementById('food-color').value; food.size = Number(document.getElementById('food-size').value); food.count = Number(document.getElementById('food-count').value); food.lifetime = clamp(Number(document.getElementById('food-lifetime').value), 20, 600); food.floatTime = clamp(Number(document.getElementById('food-float-time').value), 0, 90); food.tags = normalizeFoodTags(document.getElementById('food-tags').value); if (food.id === 'food-algae' && !food.tags.includes('pleco-only')) food.tags.push('pleco-only'); closeFoodEditor(); renderFoodPresets(); saveActivePond(); showToast(`${food.name} recipe saved`); });
...
function clearFishSelection() { state.selectedId = null; state.selectionTrail = []; state.trailAccumulator = 0; document.querySelector('.right-sidebar').classList.remove('inspector-open'); renderFishRoster(); saveActivePond(); }
document.getElementById('inspector-close').addEventListener('click', clearFishSelection);
...
let foodPanelDrag = null; const foodPanel = document.getElementById('food-panel'), foodPanelHandle = foodPanel.querySelector('.food-panel-head'); foodPanelHandle.addEventListener('pointerdown', (event) => { if (window.innerWidth <= 780 || event.target.closest('button,.info-node')) return; const rect = foodPanel.getBoundingClientRect(), parentRect = foodPanel.offsetParent.getBoundingClientRect(); foodPanelDrag = { x: event.clientX, y: event.clientY, left: rect.left, top: rect.top, parentRect }; foodPanelHandle.setPointerCapture(event.pointerId); }); foodPanelHandle.addEventListener('pointermove', (event) => { if (!foodPanelDrag) return; const { parentRect } = foodPanelDrag, maxLeft = Math.max(parentRect.left + 8, parentRect.right - foodPanel.offsetWidth - 8), maxTop = Math.max(parentRect.top + 8, parentRect.bottom - foodPanel.offsetHeight - 8), left = clamp(foodPanelDrag.left + event.clientX - foodPanelDrag.x, parentRect.left + 8, maxLeft), top = clamp(foodPanelDrag.top + event.clientY - foodPanelDrag.y, parentRect.top + 8, maxTop); foodPanel.style.left = `${left - parentRect.left}px`; foodPanel.style.top = `${top - parentRect.top}px`; foodPanel.style.right = 'auto'; foodPanel.style.bottom = 'auto'; }); foodPanelHandle.addEventListener('pointerup', () => { foodPanelDrag = null; }); foodPanelHandle.addEventListener('pointercancel', () => { foodPanelDrag = null; });
...
document.addEventListener('keydown', (event) => { const typing = event.target.matches('input,textarea,select,[contenteditable="true"]'); if (!typing && event.key.toLowerCase() === 'f') { if (!document.fullscreenElement) document.documentElement.requestFullscreen?.(); else document.exitFullscreen?.(); } if (event.key === 'Escape' && !fishCreator.hidden) closeFishCreator(); else if (event.key === 'Escape' && state.editor.active) closeDecorator(); });
document.getElementById('add-fish-button').addEventListener('click', openFishCreator);
document.getElementById('fish-creator-close').addEventListener('click', closeFishCreator); fishCreator.addEventListener('pointerdown', (event) => { if (event.target === fishCreator) closeFishCreator(); });
document.querySelectorAll('[data-create-species]').forEach((button) => button.addEventListener('click', () => { fishCreatorDraft.species = button.dataset.createSpecies; syncFishCreator(); }));
document.querySelectorAll('[data-create-color]').forEach((button) => button.addEventListener('click', () => { fishCreatorDraft.color = button.dataset.createColor; syncFishCreator(); }));
document.getElementById('fish-create-color').addEventListener('input', (event) => { fishCreatorDraft.color = event.target.value.toLowerCase(); syncFishCreator(); });
document.getElementById('fish-create-name').addEventListener('input', (event) => { fishCreatorDraft.name = event.target.value; document.getElementById('fish-create-error').textContent = ''; }); document.getElementById('fish-create-name').addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); advanceFishCreator(); } });
document.getElementById('fish-create-next').addEventListener('click', advanceFishCreator); document.getElementById('fish-create-back').addEventListener('click', () => { fishCreatorStep = Math.max(0, fishCreatorStep - 1); syncFishCreator(); }); document.getElementById('fish-create-confirm').addEventListener('click', createFishFromWizard);
...
document.getElementById('pond-context-rename').addEventListener('click', () => { const id = pondContextTargetId; closePondContextMenu(); if (id) openPondManager('rename', id); });
document.getElementById('pond-context-delete').addEventListener('click', () => { const id = pondContextTargetId; closePondContextMenu(); if (id) deletePond(id); });
document.addEventListener('pointerdown', (event) => { if (!event.target.closest('#pond-context-menu,.pond-library-item')) closePondContextMenu(); });
document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && !document.getElementById('pond-context-menu').hidden) { event.stopPropagation(); closePondContextMenu({ restoreFocus: true }); } });
window.addEventListener('resize', () => closePondContextMenu()); window.addEventListener('scroll', () => closePondContextMenu(), true);
...
function syncEditorControls() {
  const fish = selectedFish(), anatomy = fish.anatomy, finTarget = document.getElementById('fin-target').value, profile = anatomy.fins[finTarget];
  ...
  const dorsalPosition = Math.round((anatomy.fins.dorsal.position ?? .3) * 100); document.getElementById('dorsal-position-control').hidden = finTarget !== 'dorsal'; document.getElementById('dorsal-position').value = dorsalPosition; document.getElementById('dorsal-position-value').textContent = `${dorsalPosition}% from head`;
...
document.getElementById('dorsal-position').addEventListener('input', (event) => { selectedFish().anatomy.fins.dorsal.position = Number(event.target.value) / 100; document.getElementById('dorsal-position-value').textContent = `${event.target.value}% from head`; renderPaintStudio(); renderPortrait(); });
...
document.getElementById('fish-name').addEventListener('change', (event) => { const fish = selectedFish(); if (!fish) return; if (!renameFish(fish.id, event.target.value)) event.target.value = fish.name; });
...
window.render_game_to_text = () => JSON.stringify({
  ...
  fish: state.fish.map((fish) => { const width = canvas.clientWidth, height = canvas.clientHeight; const { rig, depthScale } = updateRig(fish, width, height), mouth = mouthPointNormalized(fish), lastBite = fish.cognition.memories.find((memory) => memory.type === 'fed'), friendships = Object.entries(fish.social.relationships || {}).map(([id, relation]) => ({ id, score: (relation.affinity + relation.trust + relation.familiarity) / 3 })).sort((a, b) => b.score - a.score); const shadow = fishFloorShadow(fish, rig, bodyGeometry(fish, rig, depthScale)); return { id: fish.id, name: fish.name, species: fish.species, color: fish.color, x: +fish.x.toFixed(3), y: +fish.y.toFixed(3), mouth: { x: +mouth.x.toFixed(3), y: +mouth.y.toFixed(3) }, lastBiteMouthErrorPx: lastBite?.mouthErrorPx == null ? null : +lastBite.mouthErrorPx.toFixed(2), depth: +fish.z.toFixed(2), depthBrightness: +lerp(1.02, .48, Math.pow(fish.z, .82)).toFixed(2), verticalVelocity: +fish.verticalVelocity.toFixed(3), visualScale: +depthScale.toFixed(2), heading: +fish.heading.toFixed(2), tailBeat: +fish.tailBeat.toFixed(2), swimStretch: +(fish.swimStretch || 1).toFixed(3), edgeTimer: +(fish.edgeTimer || 0).toFixed(2), attached: !!fish.attached, insideCave: caveForFish(fish)?.id || null, caveStage: fish.cognition.caveStage || null, caveRestRemaining: +Math.max(0, (fish.cognition.caveRestUntil || 0) - state.clock).toFixed(1), intent: fish.cognition.intent, socialStyle: fish.algaeEater ? 'solitary' : fish.species === 'Koi' ? 'friendship-building' : 'social', closestFriend: friendships[0] ? { id: friendships[0].id, score: +friendships[0].score.toFixed(3) } : null, boredom: +fish.physiology.boredom.toFixed(2), hunger: Math.round(fish.physiology.hunger), satiety: +fish.physiology.satiety.toFixed(2), bellyFullness: +clamp(fish.physiology.satiety * .72 + (fish.physiology.digestiveLoad ?? 0) * .68, 0, 1).toFixed(2), digestiveLoad: +(fish.physiology.digestiveLoad ?? 0).toFixed(2), nextWasteIn: +Math.max(0, (fish.physiology.nextWasteAt ?? 0) - state.clock).toFixed(1), energy: Math.round(fish.physiology.energy), morphology: { size: +fish.size.toFixed(2), ageDays: fish.ageDays, bodyLength: +fish.anatomy.bodyLength.toFixed(2), bodyWidth: +fish.anatomy.bodyWidth.toFixed(2), headPoint: +fish.anatomy.headPoint.toFixed(2), barbelLength: +fish.anatomy.barbelLength.toFixed(2) }, floorShadow: { offset: +shadow.offset.toFixed(1), blur: +shadow.blur.toFixed(1), alpha: +shadow.alpha.toFixed(3) }, finColor: fish.finColor, foodTarget: fish.cognition.foodTargetId, biteCooldown: +Math.max(0, fish.cognition.biteCooldownUntil - state.clock).toFixed(2), paintStrokes: fish.textureStrokes.length, finPaintStrokes: fish.finStrokes.length, scales: fish.anatomy.scalesEnabled, scaleSize: fish.anatomy.scaleSize, optionalFins: { ventral: fish.anatomy.ventralEnabled, anal: fish.anatomy.analEnabled, barbels: fish.anatomy.barbelsEnabled }, finProfiles: Object.fromEntries(Object.entries(fish.anatomy.fins).map(([key, value]) => [key, { length: +value.length.toFixed(2), width: +value.width.toFixed(2), ...(key === 'dorsal' ? { position: +(value.position ?? .3).toFixed(2) } : {}), edge: value.edge.map((point) => [+(point.u.toFixed(2)), +(point.v.toFixed(2))]), tip: [+(value.tip.u.toFixed(2)), +(value.tip.v.toFixed(2))] }])), rigJoints: fish.rig?.joints.length || 0, rigidSkullSegments: 3 }; }),
  foodPieces: state.foods.map((piece) => ({ id: piece.id, foodId: piece.foodId, color: piece.color, x: +piece.x.toFixed(3), y: +piece.y.toFixed(3), depth: +piece.depth.toFixed(2), age: +piece.age.toFixed(1), floatRemaining: +Math.max(0, piece.floatTime - piece.age).toFixed(1), lifetimeRemaining: +Math.max(0, piece.lifetime - piece.age).toFixed(1), visualScale: +lerp(1, .34, Math.pow(piece.depth, .78)).toFixed(2), visualOpacity: +foodOpacityAtDepth(piece.depth).toFixed(2), driftVelocity: [+(piece.vx || 0).toFixed(4), +(piece.vy || 0).toFixed(4)], claimedBy: piece.claimedBy || null })),
  foodPresets: state.foodPresets.map((food) => ({ id: food.id, name: food.name, color: food.color, size: food.size, count: food.count, lifetime: normalizeFoodPreset(food).lifetime, floatTime: food.floatTime, tags: normalizeFoodTags(food.tags), plecoOnly: food.id === 'food-algae' || normalizeFoodTags(food.tags).includes('pleco-only') })),
  ...
  ponds: { activeId: state.activePondId, items: state.ponds.map((pond) => ({ id: pond.id, name: pond.name, fishCount: pond.data.fish.length, habitat: { rocks: pond.data.decorations.filter((item) => item.type === 'rock').length, lilies: pond.data.decorations.filter((item) => item.type === 'lily').length, reeds: pond.data.decorations.filter((item) => item.type === 'reed').length } })) },
  ...
  selection: { fishId: state.selectedId, particleTrail: state.selectionTrail.length, glow: state.selectedId ? 'strong body-following bloom' : 'none' },
  ...
  ui: { headerRemoved: !document.querySelector('.stage-head'), collectionRemoved: !document.querySelector('[data-view="collection"]'), timeDialLocation: document.getElementById('time-dial').closest('.sidebar-cycle') ? 'pond-cycle section' : 'stage', sidebarSectionOrder: [...document.querySelectorAll('.left-sidebar>.side-section-label,.left-sidebar>.sidebar-cycle>.side-section-label,.left-sidebar>.sidebar-settings>.side-section-label')].map((label) => label.textContent.trim()), pondSettingsControls: [...document.querySelectorAll('.sidebar-settings .stage-control')].map((button) => button.id), fishRosterOpen: !document.getElementById('fish-roster-panel').hidden, fishRosterCount: document.querySelectorAll('.fish-roster-card').length, fishRosterGroups: Object.fromEntries([...document.querySelectorAll('.fish-roster-group')].map((group) => [group.dataset.fishGroup, group.querySelectorAll('.fish-roster-card').length])), fishCreator: { open: !fishCreator.hidden, step: fishCreatorStep + 1, species: fishCreatorDraft.species, color: fishCreatorDraft.color, name: fishCreatorDraft.name.trim() }, pondActions: 'right-click or long-press context menu', pondContextOpen: !document.getElementById('pond-context-menu').hidden, pondContextTarget: pondContextTargetId },
...
const HELP_BY_SELECTOR = {
  '[data-view="pond"]': 'Return to the live pond. Click a fish in the water to inspect it.', '#my-fish-nav': 'Open a visual list of every fish living in this pond.', '#feed-nav': 'Open the feeding tray. Choose a recipe, then click the pond to drop individual pieces.', '#clean-nav': 'Remove waste and old sunken food, reduce algae, and improve water quality.', '#decorate-nav': 'Place and directly move, scale, rotate, prune, or remove habitat objects.', '#add-fish-button': 'Open a guided setup to choose Koi or Pleco, a base color, and a name before adding the fish.', '#pond-add-button': 'Create a separate pond with its own fish, habitat, water, and saved progress.', '#export-save': 'Download all ponds and custom fish as a backup file.', '#restore-save': 'Replace this device\'s pond library with a previously exported backup.', '#performance-meter': 'Shows animation speed, render time, and the current water-render resolution.', '#time-dial': 'Drag around the circle to move smoothly through sunrise, day, sunset, and night.', '#time-speed': 'Pause the day cycle or choose how quickly pond time passes.', '#water-settings-button': 'Open controls for water color, waves, current, clarity, and pond depth.', '#fullscreen-button': 'Fill the screen with the pond. Press Escape to leave fullscreen.', '#food-panel .food-panel-head': 'Drag this heading to move the feeding tray around the pond.', '#customize-button': 'Open Fish Atelier to paint this fish and edit its anatomy and details.', '#details-button': 'Open the full editor for the selected fish.', '#remove-fish-button': 'Permanently remove this fish after a confirmation.', '#pond-generate-name': 'Create a new nature-inspired pond name.', '#pond-confirm-save': 'Save the pond name and finish creating or renaming it.'
};
const HELP_BY_LABEL = {
  'shallow color': 'The water color seen over shelves and near the pond edge.', 'deep color': 'The water color used over the deepest part of the pond.', 'surface tint': 'The color of reflected light and moving highlights on top of the water.', 'wave size': 'Changes the spacing and size of surface wave patterns.', 'surface speed': 'Controls how quickly waves and surface details move.', 'current strength': 'Controls how strongly floating food and surface details drift.', 'current direction': 'Sets the direction the surface current travels.', 'wave strength': 'Controls how strongly the water surface bends and catches light.', 'cellular sparkle': 'Adds or removes the bright, broken highlight cells on the surface.', 'refraction': 'Controls how strongly the surface visually bends submerged objects.', 'surface opacity': 'Controls how visible the crisp water surface is over fish.', 'water clarity': 'Clear water shows submerged fish more strongly; murky water hides them.', 'pond depth': 'Changes the perceived basin depth, fish depth range, shadows, and perspective.', 'color': 'The base color used for this food recipe.', 'size': 'The physical size of each individual food piece.', 'pieces': 'How many separate pieces are released with each pond click.', 'lifetime': 'How many seconds uneaten food remains before dissolving.', 'surface float': 'How long the food stays buoyant at the surface before it begins sinking.', 'food tags': 'Keywords fish use to decide whether they like or can eat this recipe.', 'scale': 'Changes the size of the selected habitat object.', 'border': 'Choose the pond-edge material and adjust how wide it appears.', 'paint target': 'Choose the body or fin surface that receives your brush strokes.', 'brush color': 'Choose the paint color applied to the selected fish surface.', 'brush': 'Changes the physical width of the painted line.', 'fin to sculpt': 'Choose which fin the numbered spline handles reshape.', 'fin color': 'Changes the unpainted base color of the fish fins.', 'length': 'Makes the selected fin longer or shorter.', 'width': 'Makes the selected fin wider or narrower.', 'dorsal start': 'Moves the dorsal fin attachment forward or backward along the spine.', 'body length': 'Changes the fish from compact to long without changing its age.', 'body width': 'Changes how slim or broad the fish body is.', 'head point': 'Changes how rounded or pointed the nose is.', 'barbel length': 'Changes how far the koi barbels trail behind the mouth.', 'fish name': 'Rename this fish. The new name is saved automatically.', 'eye size': 'Changes the size of both eyes.', 'scale size': 'Changes the spacing and size of the optional scale overlay.', 'fish size': 'Changes the fish\'s overall physical size and collision body.', 'age': 'Sets the fish age shown in its details and life stage.', 'liked food tags': 'Keywords this fish prefers when choosing among available foods.', 'pond name': 'The saved name shown in your pond library.'
};
function installInfoTooltips() {
  const tooltip = document.getElementById('plain-tooltip');
  const show = (node) => { tooltip.textContent = node.dataset.help; tooltip.hidden = false; const rect = node.getBoundingClientRect(), width = Math.min(260, window.innerWidth - 20), left = clamp(rect.left + rect.width / 2 - width / 2, 10, window.innerWidth - width - 10), top = rect.bottom + 8 + Math.min(0, window.innerHeight - (rect.bottom + 90)); tooltip.style.width = `${width}px`; tooltip.style.left = `${left}px`; tooltip.style.top = `${Math.max(8, top)}px`; };
  const hide = () => { tooltip.hidden = true; };
  const add = (target, help) => { if (!target || !help || target.querySelector(':scope > .info-node,:scope > .control-caption > .info-node')) return; const node = document.createElement('span'); node.className = 'info-node'; node.tabIndex = 0; node.setAttribute('role', 'button'); node.setAttribute('aria-label', `Information: ${help}`); node.dataset.help = help; node.textContent = 'i'; if (target.tagName === 'LABEL') { const textNode = [...target.childNodes].find((child) => child.nodeType === Node.TEXT_NODE && child.textContent.trim()); if (textNode) { const caption = document.createElement('span'); caption.className = 'control-caption'; caption.textContent = textNode.textContent.trim(); textNode.replaceWith(caption); caption.appendChild(node); } else target.prepend(node); } else target.appendChild(node); node.addEventListener('pointerenter', () => show(node)); node.addEventListener('focus', () => show(node)); node.addEventListener('pointerdown', (event) => { event.stopPropagation(); }); node.addEventListener('click', (event) => { event.preventDefault(); event.stopPropagation(); show(node); }); };
  Object.entries(HELP_BY_SELECTOR).forEach(([selector, help]) => document.querySelectorAll(selector).forEach((target) => add(target, help)));
  document.querySelectorAll('.water-settings-grid label,.food-editor label,.decor-scale,.paint-control,.anatomy-grid label,.pond-manager-card>label').forEach((label) => { const textNode = [...label.childNodes].find((child) => child.nodeType === Node.TEXT_NODE && child.textContent.trim()), key = textNode?.textContent.trim().toLowerCase(); add(label, HELP_BY_LABEL[key] || `Adjust ${key || 'this setting'}. Changes are saved with the current pond.`); });
  document.querySelectorAll('.decor-tool,[title]').forEach((target) => { if (target.closest('.info-node')) return; const help = target.title; if (help && !target.matches('input')) add(target, `${help}. This control affects the current pond.`); });
  if (!document.documentElement.dataset.infoDismissReady) { document.documentElement.dataset.infoDismissReady = 'true'; document.addEventListener('pointerdown', (event) => { if (!event.target.closest('.info-node')) hide(); }); }
}
...
async function bootstrap() { await initializePondLibrary(); renderFoodPresets(); syncWaterControls(); updateTimeDial(); syncInspector(); installInfoTooltips(); requestAnimationFrame(animate); }
bootstrap();

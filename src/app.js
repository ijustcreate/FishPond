...
function applyPondData(data) {
  ...
  state.fish.forEach((fish, index) => { fish.foodTags = normalizeFoodTags(Array.isArray(fish.foodTags) ? fish.foodTags : defaultFishFoodTags(fish.species, fish.algaeEater)); fish.physiology.digestiveLoad ??= .08; fish.physiology.nextWasteAt ??= 32 + seeded(index + 301) * 38; fish.cognition.hideCaveId ??= null; fish.anatomy.barbelLength ??= 1; fish.ambientTrailAccumulator = 0; }); state.foodPresets.forEach(normalizeFoodPreset); state.foods.forEach((piece) => { const preset = state.foodPresets.find((food) => food.id === piece.foodId); const timing = preset || defaultFoodTiming(piece.foodId); piece.floatTime ??= timing.floatTime; piece.lifetime ??= timing.lifetime; piece.sinkRate ??= 1 / (timing.sinkDuration || 60); }); state.ambientTrails = [];
